...
class RestaurantLogic
{
    ...
    {
        return Restaurant::with('discount', 'campaigns')->whereId($restaurant_id)->first();
    }
    ...
    public static function search_restaurants($name, $zone_id, $limit = 10, $offset = 1)
    {
        $key = explode(' ', $name);
        $paginator = Restaurant::with('discount')->where('zone_id', $zone_id)->where(function ($q) use ($key) {
