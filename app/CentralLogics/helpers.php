...
class Helpers
{
    ...
    public static function product_data_formatting($data, $multi_data = false)
    {
        ...
        if ($multi_data == true) {
            foreach ($data as $item) {
                ...
                $item['restaurant_discount'] = $item->restaurant->discount?$item->restaurant->discount->discount:0;
                ...
                $item['rating_count'] = (double)(!empty($item->rating[0])?$item->rating[0]->rating_count:0);
            ...
            $data['restaurant_discount'] = $data->restaurant->discount?$data->restaurant->discount->discount:0;
    ...
    public static function order_data_formatting($data, $multi_data = false)
    {
        ...
        else
        {
            ...
            $data['item_count'] = $data->details->count();
            unset($data['details']);
        }
        return $data;
    }
    ...
    public static function deliverymen_data_formatting($data)
    {
        $storage = [];
        foreach ($data as $item) {
            $item['avg_rating']=count($item->rating)?(float)$item->rating[0]->average:0;
            $item['rating_count']=count($item->rating)?$item->rating[0]->rating_count:0;
    ...
    public static function employee_module_permission_check($mod_name)
    {
        if (auth('vendor')->check()) {
            if($mod_name == 'food')
            {
                return auth('vendor')->user()->restaurants[0]->food_section;
            }
            return true;
        }
        else if(auth('vendor_employee')->check())
        {
            ...
            if (isset($permission) && in_array($mod_name, (array)json_decode($permission)) == true) {
                if($mod_name == 'food')
                {
                    return (boolean)auth('vendor_employee')->user()->restaurant->food_section;
                }
                return true;
            }
        }
    ...
    public static function calculate_addon_price($addons,$add_on_qtys)
    {
        ...
        if($addons)
        {
            foreach($addons as $key2 =>$addon)
            {
                ...
                $data[] = ['name'=>$addon->name, 'price'=>$addon->price, 'quantity'=> $add_on_qty];
