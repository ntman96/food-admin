...
class OrderLogic
{
    ...
    public static function refund_order($order)
    {
        ...
        {
            DB::beginTransaction();
            if($received_by=='admin' && $order->delivery_man_id)
            {
                if($order->payment_method="cash_on_delivery")
                {
                    $adminWallet->collected_cash = $adminWallet->collected_cash-$order_transaction->order_amount;
                }
                else
                {
                    $adminWallet->digital_received = $adminWallet->digital_received-$order_transaction->order_amount;
                }
                
            }
            else if($received_by=='restaurant')
            {
                $vendorWallet->collected_cash = $vendorWallet->collected_cash - $order_transaction->order_amount;
            }
                ...
                //     'from_id'=>$order->user_id,
                //     'current_balance'=> 0,
                //     'amount'=> $order_transaction->order_amount,
                //     'method'=>'CASH',
                //     'created_at' => now(),
                //     'updated_at' => now()
                // ]);
            ...
            else if($received_by=='deliveryman')
            {
                ...
                $dmWallet->collected_cash=$dmWallet->collected_cash-$order_transaction->order_amount;
                $dmWallet->save();
            }
            $adminWallet->save();
            $vendorWallet->save();
